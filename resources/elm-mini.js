...
Elm.Website.Docs=function(){
 ...
 try{if (!(Elm.List instanceof Object)) throw 'module not found'; } catch(e) {throw "Module 'List' is missing. Compile with --make flag or load missing module in a separate JavaScript file.";}
 var intersperse=Elm.List.intersperse;
 var zipWith=Elm.List.zipWith;
 try{if (!(Elm.Website.ColorScheme instanceof Object)) throw 'module not found'; } catch(e) {throw "Module 'Website.ColorScheme' is missing. Compile with --make flag or load missing module in a separate JavaScript file.";}
 var hiddenVars=[];
 for(var i in Elm.Website.ColorScheme){
  if (hiddenVars.indexOf(i) >= 0) continue;
  this[i]=Elm.Website.ColorScheme[i];}
 ...
 function topBar_1(k_8){
  return function(n_9){
   return function(){
    var n__10=toFloat(n_9);
    var k__11=toFloat(k_8);
    var segs_12=map(function(i_15){
     return round((n__10*(toFloat(i_15)/k__11)));})(function(){
     var lo=1;
     var hi=k_8;
     var lst=["Nil"];if(lo<=hi){do{
     var lst=["Cons",hi,lst];}while(hi-->lo)}
     return lst;}());
    var ws_13=zipWith(function(x_16){
     return function(y_17){
      return (x_16-y_17);};})(segs_12)(["Cons",0,segs_12]);
    var accentCycle_14=concatMap(function(__18){
     ...
     var lst=["Cons",hi,lst];}while(hi-->lo)}
     return lst;}());
    return flow(right)(zipWith(function(c_19){
     return function(w_20){
      return color(c_19)(spacer(w_20)(5));};})(accentCycle_14)(ws_13));}();};};
 function skeleton_2(body_21){
  return function(outer_22){
   return function(){
    var content_23=body_21((outer_22-80));
    return flow(down)(["Cons",topBar_1(10)(outer_22),["Cons",spacer(outer_22)(15),["Cons",container(outer_22)(heightOf(content_23))(midTop)(content_23),["Cons",function(x){
     return container(outer_22)(50)(midBottom)(text(x));}(Value.append(Text.color(rgb(145)(145)(145))(toText("&copy; 2011-2012 ")),Text.link(Value.str("https://github.com/evancz"))(toText("Evan Czaplicki")))),["Nil"]]]]]);}();};};
 function addSpaces_3(px_24){
  return intersperse(spacer(1)(px_24));};
 function section_4(s_25){
  return function(x){
   return bold(Text.height(s_25)(toText(x)));};};
 function entry_5(w_26){
  return function(Tuple3$nametypdesc_27){
   return function(){
   switch(Tuple3$nametypdesc_27[0]){
    case "Tuple3":
    return function(){
     var colons_31=Text.color(accent1)(toText(" :: "));
     return function(){
      var tipe_32=((compare(length(Tuple3$nametypdesc_27[2]))(0)[0] === 'GT')?Value.append(colons_31,toText(Tuple3$nametypdesc_27[2])):toText(""));
      return flow(down)(["Cons",color(mediumGrey)(spacer(w_26)(1)),["Cons",function(x){
       return width(w_26)(color(lightGrey)(text(monospace(x))));}(Value.append(bold(toText(Tuple3$nametypdesc_27[1])),tipe_32)),["Cons",spacer(1)(10),["Cons",flow(right)(["Cons",spacer(50)(10),["Cons",width((w_26-50))(plainText(Tuple3$nametypdesc_27[3])),["Nil"]]]),["Cons",spacer(1)(20),["Nil"]]]]]]);}();}();
   }
   throw "Non-exhaustive pattern match in case";}();};};
 function group_6(w_33){
  return function(Tuple2$namefs_34){
   return function(){
   switch(Tuple2$namefs_34[0]){
    case "Tuple2":
    return flow(down)(["Cons",text(section_4((5/4))(Tuple2$namefs_34[1])),["Cons",spacer(1)(20),map(entry_5(w_33))(Tuple2$namefs_34[2])]]);
   }
   throw "Non-exhaustive pattern match in case";}();};};
 function createDocs_7(name_37){
  return function(cats_38){
   return function(){
    function f_39(w_40){
     return flow(down)(["Cons",text(Text.link(Value.str("/Documentation.elm"))(toText("Back"))),["Cons",function(x){
      return width(w_40)(centeredText(x));}(section_4(2)(name_37)),["Cons",spacer(1)(30),addSpaces_3(50)(map(group_6(w_40))(cats_38))]]]);};
    return lift(skeleton_2(f_39))(Window.width);}();};};
 return {createDocs:createDocs_7};}();
Elm.main=function(){
 return Elm.Website.Docs.main;};
} catch (e) {Elm.main=function() {var msg = ('<br/><h2>Your browser may not be supported. Are you using a modern browser?</h2>' + '<br/><span style="color:grey">Runtime Error in Website.Docs module:<br/>' + e + '<br/><br/>The problem may stem from an improper usage of:<br/>Text.color, Text.link, Window.width, accent0, accent1, accent2, accent3, accent4, lightGrey, mediumGrey</span>');document.body.innerHTML = Text.monospace(msg);throw e;};}
try{
...
Elm.Website.Tiles=function(){
 ...
 function format_0(Tuple3$xyz_5){
  return function(){
  switch(Tuple3$xyz_5[0]){
   case "Tuple3":
   return ["Tuple3",Tuple3$xyz_5[1],Value.append(Tuple3$xyz_5[3],Value.append(Tuple3$xyz_5[2],Value.str(".elm"))),Value.append(Value.str("/screenshot/"),Value.append(Tuple3$xyz_5[2],Value.str(".jpg")))];
  }
  throw "Non-exhaustive pattern match in case";}();};
 function toTile_2(info_9){
  return function(){
   var Tuple3$nameexpic_10=format_0(info_9);
   var name_11=function(){
   switch(Tuple3$nameexpic_10[0]){
    case "Tuple3":
    return Tuple3$nameexpic_10[1];
   }
   throw "Non-exhaustive pattern match in case";}();
   var ex_12=function(){
   switch(Tuple3$nameexpic_10[0]){
    case "Tuple3":
    return Tuple3$nameexpic_10[2];
   }
   throw "Non-exhaustive pattern match in case";}();
   var pic_13=function(){
   switch(Tuple3$nameexpic_10[0]){
    case "Tuple3":
    return Tuple3$nameexpic_10[3];
   }
   throw "Non-exhaustive pattern match in case";}();
   var x_14=tileSize_1;
 ...
 function tile_4(w_28){
  return function(tiles_29){
   return function(x){
    return flow(down)(intersperse(spacer(1)(14))(map(flow(right))(x)));}(groups_3(div(w_28)(tileSize_1))(tiles_29));};};
 return {toTile:toTile_2,tile:tile_4};}();
Elm.main=function(){
 return Elm.Website.Tiles.main;};
} catch (e) {Elm.main=function() {var msg = ('<br/><h2>Your browser may not be supported. Are you using a modern browser?</h2>' + '<br/><span style="color:grey">Runtime Error in Website.Tiles module:<br/>' + e + '</span>');document.body.innerHTML = Text.monospace(msg);throw e;};}
try{
...
Elm.Website.Skeleton=function(){
 ...
 function button_0(Tuple3$namehrefclr_5){
  return function(){
  switch(Tuple3$namehrefclr_5[0]){
   case "Tuple3":
   return function(){
    var accent_9=color(Tuple3$namehrefclr_5[3])(spacer(100)(2));
    return function(){
     var butn_10=container(100)(58)(middle)(function(x){
      return text(Text.color(black)(x));}(toText(Tuple3$namehrefclr_5[1])));
     return link(Tuple3$namehrefclr_5[2])(below(accent_9)(butn_10));}();}();
  }
  throw "Non-exhaustive pattern match in case";}();};
 function title_2(w_11){
  return function(){
   var elm_12=function(x){
    return text(Text.link(Value.str("/"))(Text.color(black)(Text.height(2)(bold(x)))));}(toText("Elm"));
   return container(w_11)(60)(midLeft)(elm_12);}();};
 function heading_3(outer_13){
  return function(inner_14){
   return function(){
    var header_15=container(outer_13)(60)(middle)(beside(title_2((inner_14-widthOf(buttons_1))))(buttons_1));
    return layers(["Cons",flow(down)(["Cons",color(lightGrey)(spacer(outer_13)(58)),["Cons",color(mediumGrey)(spacer(outer_13)(1)),["Nil"]]]),["Cons",header_15,["Nil"]]]);}();};};
 function skeleton_4(bodyFunc_16){
  return function(outer_17){
   return function(){
    var inner_18=((compare(outer_17)(820)[0] === 'LT')?(outer_17-20):800);
    return function(){
     var body_19=bodyFunc_16(inner_18);
     return flow(down)(["Cons",heading_3(outer_17)(inner_18),["Cons",spacer(outer_17)(10),["Cons",container(outer_17)(heightOf(body_19))(middle)(body_19),["Cons",function(x){
      return container(outer_17)(50)(midBottom)(text(x));}(Value.append(Text.color(rgb(145)(145)(145))(toText("&copy; 2011-2012 ")),Text.link(Value.str("https://github.com/evancz"))(toText("Evan Czaplicki")))),["Nil"]]]]]);}();}();};};
 return {skeleton:skeleton_4};}();
Elm.main=function(){
 return Elm.Website.Skeleton.main;};
} catch (e) {Elm.main=function() {var msg = ('<br/><h2>Your browser may not be supported. Are you using a modern browser?</h2>' + '<br/><span style="color:grey">Runtime Error in Website.Skeleton module:<br/>' + e + '<br/><br/>The problem may stem from an improper usage of:<br/>Text.color, Text.link, accent1, accent2, accent3, accent4, below, beside, lightGrey, mediumGrey</span>');document.body.innerHTML = Text.monospace(msg);throw e;};}