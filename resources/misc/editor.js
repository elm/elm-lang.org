...
var elmSyntax = {
	...
	':':
		'<a href="/learn/Understanding-Types.elm" target="_blank">' +
		'type annotations</a>, pronounced &ldquo;has type&rdquo;',
	'->': {
		message:
			'&ldquo;function&rdquo; in type annotations <i>or</i> ' +
	...
	'as':
		'aliasing. Can be used on imported modules and pattern complex patterns.',
	'let':
		'beginning a let expression',
	'in':
		'marking the end of a block of definitions, and starting an expression',
	'if':
		'beginning an conditional expression',
	'then':
		'separating the conditional from the first branch',
	'else':
		'separating the first and second branch',
	'case':
		'beginning a case expression',
	'of':
		'separating the expression to be pattern matched from possible case branches',
	'type':
		'defining a <a href="/learn/Union-Types.elm" target="_blank">union type</a> ' +
		'or type aliases',
	'_' :
		'pattern match anything, often called a &ldquo;wildcard&rdquo;',
	'..':
		'number interpolation',
	'|' :
		'separating various things, sometimes pronounced &ldquo;where&rdquo;',
	'main':
		'showing values on screen, must have type <code>Element</code> or <code>Signal Element</code>',
	'import':
		'importing modules',
	'infix' :
		'declaring that a given operator is non-associative',
	'infixl':
		'declaring that a given operator is left-associative',
	'infixr':
		'declaring that a given operator is right-associative',
	'module':
		'declaring a module definition',
};
...
function parseDoc(modules)
{
	...
	function parseModule(module)
	{
		...
		return []
			.concat(module.values, module.datatypes, module.aliases)
			.map(formatEntry);
	}
	return {
...
function openDocPage()
{
	...
	if (result.isSyntax)
	{
		return window.open('/learn/Syntax.elm','_blank');
	}
	if (result.isModule)
	{
		return window.open(docsHref(result.home), '_blank');
	}
...
function formatType(result)
{
	...
	if (firstFive === 'type ' || firstFive === 'data ')
	{
		...
		var whitelist = [
			'Maybe','Either','Response','Order','Action',
...
function lookupDocs(token, line)
{
	...
	if (token.string)
	{
		...
		if (results.length === 1)
		{
			return { isModule:true, home:results[0].name, desc:results[0].desc };
		}
	}
	return null;
}
