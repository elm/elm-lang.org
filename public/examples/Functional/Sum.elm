...
Overview:
  foldl and foldr are two types of folds. They are read
  'fold left' and 'fold right'. Each function takes three
  arguments:
    - combining function
  ...
  an element from the list, and the accumulated result so far,
  and puts them together. In foldl we traverse the list starting
  on the left. foldr starts on the right. Each element is
  combined with the accumulator until the whole list has been
  traversed.
