...
#### Breaking Changes:
  ...
  * Overhaul the [`Graphics.Input`][gi] library (inspired by
    [Spiros Eliopoulos](http://github.com/seliopou) and
  ...
  * Overhaul `Regex` library (inspired by [Attila Gazso](https://github.com/agazso))
...
#### Improvements:
  ...
  * Add [`Debug`](http://library.elm-lang.org/catalog/evancz-Elm/0.12/Debug)
    ...
    debugging!
  * Drastically improved performance on markdown parsing. Thanks to [Daniël
    Heres](https://github.com/Dandandan) for discovering that we were appending
    lists in the wrong direction!
  * Add `Date.fromTime` function
  * Use pointer-events to detect hovers on layered elements (thanks to
    [@Xashili](https://github.com/xashili))
